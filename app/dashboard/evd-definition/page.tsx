    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-red-600 via-red-700 to-red-800 rounded-2xl p-8 text-white relative overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2">Ebola Virus Disease (EVD)</h1>
              <p className="text-red-100 text-lg">Case Definitions and Clinical Guidelines</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {caseDefinitions.map((definition, index) => (
          <Card key={definition.id} className="hover:shadow-lg transition-all duration-300">
            <CardHeader>
              <div className="flex items-center space-x-4">
                <div className={`h-12 w-12 ${definition.color} rounded-xl flex items-center justify-center shadow-lg`}>
              ...
              <ScrollArea className="h-96">
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-gray-900 mb-2">Primary Criteria:</h4>
                    <p className="text-sm text-gray-700">{definition.criteria.primary}</p>
                  </div>

                  {/* Additional Criteria */}
                  {definition.criteria.and && (
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">AND {definition.criteria.and}</h4>
                      {definition.criteria.signs && (
                        <div className="grid grid-cols-1 gap-2">
                          {definition.criteria.signs.map((sign, idx) => (
                  ...
                  {definition.criteria.or && (
                    <div>
                      ...
                      <h4 className="font-semibold text-gray-900 mb-3">OR any of the following:</h4>
                      <div className="space-y-3">
                        {definition.criteria.or.map((item, idx) => (
                          <div key={idx} className="bg-blue-50 p-3 rounded-lg">
                            {typeof item === "string" ? (
                              <p className="text-sm text-gray-700">{item}</p>
                            ) : (
                              <div>
                                <p className="text-sm font-medium text-gray-900 mb-2">{item.condition}</p>
                                <div className="grid grid-cols-1 gap-1 ml-4">
                                  {item.signs.map((sign, signIdx) => (
                                    <div key={signIdx} className="flex items-center space-x-2">
                  ...
                  {definition.criteria.tests && (
                    <div>
                      <Separator className="my-4" />
                      <h4 className="font-semibold text-gray-900 mb-3">Laboratory Tests:</h4>
                      <div className="space-y-2">
                        {definition.criteria.tests.map((test, idx) => (
                          <div key={idx} className="flex items-center space-x-2 bg-green-50 p-2 rounded">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-red-700">
                <li className="flex items-start space-x-2">
                  ...
                  <span>Isolate suspected cases immediately</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Use appropriate PPE for all interactions</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Report to health authorities within 24 hours</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Collect samples for laboratory testing</span>
                </li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-red-800 mb-3">Contact Tracing:</h4>
              <ul className="space-y-2 text-sm text-red-700">
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Identify all close contacts in past 21 days</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Monitor contacts for symptoms daily</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Document all contact information</span>
                </li>
                <li className="flex items-start space-x-2">
                  <div className="h-1.5 w-1.5 bg-red-600 rounded-full mt-2"></div>
                  <span>Provide health education to contacts</span>
                </li>
              </ul>
            </div>
          </div>
